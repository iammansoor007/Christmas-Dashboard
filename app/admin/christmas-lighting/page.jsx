...
export default function ChristmasLightingEditor() {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('/api/admin/christmas-lighting', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold">Edit Christmas Lighting Section</h1>
          <button
            onClick={handleSave}
            disabled={saving}
        ...
        <div className="bg-white rounded-lg shadow p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={data.cta?.primary?.text || 'Get My Free Quote'}
                    onChange={(e) => setData({
                      ...data,
                      cta: { ...data.cta, primary: { ...data.cta?.primary, text: e.target.value } }
                    })}
                    className="w-full px-3 py-2 border rounded"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    value={data.cta?.primary?.link || '#freequote'}
                    onChange={(e) => setData({
                      ...data,
                      cta: { ...data.cta, primary: { ...data.cta?.primary, link: e.target.value } }
                    })}
                    className="w-full px-3 py-2 border rounded"
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={data.cta?.secondary?.text || 'View Gallery'}
                    onChange={(e) => setData({
                      ...data,
                      cta: { ...data.cta, secondary: { ...data.cta?.secondary, text: e.target.value } }
                    })}
                    className="w-full px-3 py-2 border rounded"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    value={data.cta?.secondary?.link || '/gallery'}
                    onChange={(e) => setData({
                      ...data,
                      cta: { ...data.cta, secondary: { ...data.cta?.secondary, link: e.target.value } }
                    })}
                    className="w-full px-3 py-2 border rounded"
                    placeholder="/gallery"
                  />
          ...
          {/* Image */}
          <div className="space-y-4 border-t pt-4">
            <h2 className="text-xl font-semibold">Owner Image</h2>
