...
export default function PrivacyPageEditor() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (response.ok) {
                setMessage({ type: 'success', text: 'Privacy page saved successfully!' });
            } else {
            ...
            <div className="space-y-8">
                ...
                <div className="border rounded-lg p-6 bg-white shadow">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input type="text" value={data?.hero?.badge || 'Privacy Policy'} onChange={(e) => setData({ ...data, hero: { ...data?.hero, badge: e.target.value } })} className="w-full border rounded px-3 py-2" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Title</label>
                            <input type="text" value={data?.hero?.title || 'Privacy Policy'} onChange={(e) => setData({ ...data, hero: { ...data?.hero, title: e.target.value } })} className="w-full border rounded px-3 py-2" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Subtitle</label>
                            <input type="text" value={data?.hero?.subtitle || 'How we collect, use, and protect your personal information'} onChange={(e) => setData({ ...data, hero: { ...data?.hero, subtitle: e.target.value } })} className="w-full border rounded px-3 py-2" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Last Updated Text</label>
                            <input type="text" value={data?.hero?.lastUpdatedText || 'March 2026'} onChange={(e) => setData({ ...data, hero: { ...data?.hero, lastUpdatedText: e.target.value } })} className="w-full border rounded px-3 py-2" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Background Image</label>
                            {data?.hero?.backgroundImage && <img src={data.hero.backgroundImage} alt="Hero" className="w-48 h-32 object-cover rounded mb-2" />}
                            <div className="flex gap-2">
                                <input type="text" value={data?.hero?.backgroundImage || ''} onChange={(e) => setData({ ...data, hero: { ...data?.hero, backgroundImage: e.target.value } })} className="flex-1 border rounded px-3 py-2" />
                                <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, 'hero')} disabled={uploading} className="border rounded px-3 py-2 file:mr-2 file:py-1 file:px-3 file:text-sm" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="border rounded-lg p-6 bg-white shadow">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium mb-1">Title</label>
                            <input type="text" value={data?.introduction?.title || 'Our Commitment to Privacy'} onChange={(e) => setData({ ...data, introduction: { ...data?.introduction, title: e.target.value } })} className="w-full border rounded px-3 py-2" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Description</label>
                            <textarea value={data?.introduction?.description || ''} onChange={(e) => setData({ ...data, introduction: { ...data?.introduction, description: e.target.value } })} rows="4" className="w-full border rounded px-3 py-2" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Icon</label>
                            <input type="text" value={data?.introduction?.icon || 'GiFruitTree'} onChange={(e) => setData({ ...data, introduction: { ...data?.introduction, icon: e.target.value } })} className="w-full border rounded px-3 py-2" />
                        </div>
                    </div>
                </div>
                ...
                <div className="border rounded-lg p-6 bg-white shadow">
                    <div className="flex justify-between items-center mb-4">
                        <h2 className="text-2xl font-semibold">Privacy Sections</h2>
                        <button onClick={addSection} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 flex items-center gap-2">
                    ...
                    {data?.sections?.map((section, sIndex) => (
                        <div key={sIndex} className="border rounded-lg p-4 mb-4 bg-gray-50">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => moveSection(sIndex, 'up')} disabled={sIndex === 0} className="p-1 rounded text-gray-600 hover:bg-gray-200"><FaArrowUp size={14} /></button>
                                    <button onClick={() => moveSection(sIndex, 'down')} disabled={sIndex === data.sections.length - 1} className="p-1 rounded text-gray-600 hover:bg-gray-200"><FaArrowDown size={14} /></button>
                                    <button onClick={() => removeSection(sIndex)} className="p-1 text-red-500 hover:bg-red-50 rounded"><FaTrash size={14} /></button>
                                </div>
                            </div>

                            <div className="grid grid-cols-2 gap-4 mb-4">
                                <div><label className="block text-sm font-medium mb-1">Icon</label><input type="text" value={section.icon || ''} onChange={(e) => updateSection(sIndex, 'icon', e.target.value)} className="w-full border rounded px-3 py-2" /></div>
                                <div><label className="block text-sm font-medium mb-1">Title</label><input type="text" value={section.title || ''} onChange={(e) => updateSection(sIndex, 'title', e.target.value)} className="w-full border rounded px-3 py-2" /></div>
                            </div>
                            ...
                            <div className="ml-6 mt-4 border-l-2 border-gray-200 pl-4">
                                <div className="flex justify-between items-center mb-2"><h4 className="font-medium">Subsections</h4><button onClick={() => addSubSection(sIndex)} className="px-3 py-1 bg-blue-500 text-white rounded text-sm">+ Add Subsection</button></div>
                                {section.subSections?.map((sub, subIndex) => (
                                    <div key={subIndex} className="border rounded-lg p-3 mb-3 bg-white">
                                        <div className="flex justify-between items-center mb-2"><h5 className="font-medium">Subsection {subIndex + 1}</h5><button onClick={() => removeSubSection(sIndex, subIndex)} className="text-red-500 text-sm">Remove</button></div>
                ...
                <div className="border rounded-lg p-6 bg-white shadow">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="block text-sm font-medium mb-1">Title</label><input type="text" value={data?.contactInfo?.title || 'Privacy Questions? Contact Us'} onChange={(e) => setData({ ...data, contactInfo: { ...data?.contactInfo, title: e.target.value } })} className="w-full border rounded px-3 py-2" /></div>
                        ...
                        <div><label className="block text-sm font-medium mb-1">Email</label><input type="email" value={data?.contactInfo?.email || 'privacy@christmaslightsovercolumbus.com'} onChange={(e) => setData({ ...data, contactInfo: { ...data?.contactInfo, email: e.target.value } })} className="w-full border rounded px-3 py-2" /></div>
                        ...
                        <div><label className="block text-sm font-medium mb-1">Response Time</label><input type="text" value={data?.contactInfo?.responseTime || '2-3 business days'} onChange={(e) => setData({ ...data, contactInfo: { ...data?.contactInfo, responseTime: e.target.value } })} className="w-full border rounded px-3 py-2" /></div>
                    </div>
                </div>
                ...
                <div className="border rounded-lg p-6 bg-white shadow">
                    ...
                    <div><label className="block text-sm font-medium mb-1">Meta Title</label><input type="text" value={data?.seo?.metaTitle || 'Privacy Policy | Christmas Lights Over Columbus'} onChange={(e) => setData({ ...data, seo: { ...data?.seo, metaTitle: e.target.value } })} className="w-full border rounded px-3 py-2" /></div>
