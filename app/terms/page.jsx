...
const getIcon = (iconName) => {
    if (!iconName) return FaFileContract;
    ...
    return (
        <main className="min-h-screen bg-gray-50">
            ...
            <section className="relative bg-gradient-to-r from-emerald-900 to-emerald-800 py-16 overflow-hidden">
                <div className="absolute inset-0 opacity-10"><div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_30%_50%,rgba(255,255,255,0.2)_1px,transparent_1px)] bg-[length:20px_20px]"></div></div>
                ...
                <div className="relative container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="max-w-4xl mx-auto text-center">
                        <div className="inline-flex items-center justify-center w-16 h-16 bg-white/10 backdrop-blur-sm rounded-2xl mb-6 border border-white/20">
                            {getIcon(hero?.icon || 'FaFileContract') && React.createElement(getIcon(hero?.icon || 'FaFileContract'), { className: "w-8 h-8 text-white" })}
                        </div>
                        <h1 className="font-montserrat font-bold text-3xl sm:text-4xl md:text-5xl text-white mb-4">{hero?.title || 'Terms & Conditions'}</h1>
                        <p className="text-lg text-emerald-100 max-w-2xl mx-auto">{hero?.subtitle || 'Please read these terms carefully before using our services'}</p>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
                        ...
                        {contactInfo && (
                            <div className="p-6 sm:p-8 bg-gray-50 border-t border-gray-200">
                                <h3 className="text-lg font-semibold text-gray-900 mb-4">{contactInfo.title || 'Questions? Contact Us'}</h3>
